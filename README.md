...
The DCS installation directory is derived from the configured `DCS.exe`; Groundcrew searches its directory and parent installation directories for `Scripts\MissionScripting.lua`. The gRPC management dialog shows the resolved Saved Games and DCS-installation paths separately. Every attempt is recorded in `C:\ProgramData\Groundcrew\Logs\dcs-grpc-installer.log`, including candidate loader paths, download validation, installed targets, rollback, and the exact exception. The last 250 entries can be read directly from the dialog.

Groundcrew's dashboard remains a Windows service, but DCS itself is launched in the active signed-in Windows desktop session. This gives DCS the correct user profile and Saved Games directory and permits its normal window/splash screen to appear. A Windows user must therefore be signed in on the server host when starting DCS; the session may subsequently be locked.
